...
import com.automonia.core.annotation.WTEnum;
...
import com.automonia.core.model.tuple.Tuple;
import com.automonia.core.model.tuple.TwoTuple;
import com.automonia.core.query.WTOrder;
import com.automonia.core.query.WTQuery;
import com.automonia.core.tools.DateUtils;
...
public class DataBaseFactory {
     ...
     * <p>
     * <p>
     * //        for (String fieldName : entityClassInfo.getFieldMappingColumnMap().keySet()) {
     ...
     * //                sqlValue.append(entitySimpleName).append(".").append(entityClassInfo.getFieldMappingColumnMap().get(fieldName)).append(" ").append(fieldName).append(",");
     * //            }
     * //        }
     *
    ...
    public <M extends WTModel> String getSqlOfSelectList(List<String> fieldNames, WTQuery query, Class<M> modelClass) throws NoSuchFieldException, ClassNotFoundException, WTException {
        ...
        WTEntityClassInfo entityClassInfo = getEntityClassInfoFactory().getEntityClassInfo(modelClass);

        StringBuilder sqlValue = new StringBuilder("select ");
        String entitySimpleName = entityClassInfo.getEntityClass().getSimpleName();
         ...
         */
        // 是否将entityClass类的属性用于生成select item内容的前提条件是：fieldNames为空 或者fieldNames包括了该属性
        entityClassInfo.getFieldMappingColumnMap().keySet().stream().filter(fieldName -> selectItemsIsEmpty || fieldNames.contains(fieldName)).forEach(fieldName -> sqlValue.append(entitySimpleName).append(".").append(entityClassInfo.getFieldMappingColumnMap().get(fieldName)).append(" ").append(fieldName).append(","));

        /*
        modelClass是实体扩展类, 以下只遍历实体类的数据扩展类
         */
        if (modelClass != entityClassInfo.getEntityClass()) {
         ...
         */
        sqlValue.append(" from ").append(entityClassInfo.getTableName()).append(" ").append(entitySimpleName);
        ...
        second: 关联语句
         */
        TwoTuple whereAndLeftJoinItemsSql = getSqlWhereItems(query, entityClassInfo, sqlOfLeftJoinItems, true, true, true);
    ...
    public String getSqlOfInsert(WTModel model) throws NoSuchFieldException, WTException, ClassNotFoundException {
        ...
        WTEntityClassInfo entityClassInfo = getEntityClassInfoFactory().getEntityClassInfo(model.getClass());

        StringBuilder sql = new StringBuilder("insert into ").append(entityClassInfo.getTableName());

        StringBuilder insertIntoColumnItemSql = new StringBuilder();
        StringBuilder insertIntoValueItemSql = new StringBuilder();

        for (String fieldItem : entityClassInfo.getFieldMappingColumnMap().keySet()) {
            TwoTuple<Object, Class> valueInfo = getFieldValue(model, fieldItem);
            if (valueInfo == null || valueInfo.first == null) {
                continue;
            }

            insertIntoColumnItemSql.append(",").append(entityClassInfo.getFieldMappingColumnMap().get(fieldItem));
    ...
    public String getSqlOfUpdate(WTQuery query, WTModel model) throws NoSuchFieldException, ClassNotFoundException, WTException {
        ...
        StringBuilder sqlValue = new StringBuilder("update ").append(entityClassInfo.getTableName()).append(" ").append(entitySimpleName);

        TwoTuple whereAndLeftJoinItemsSql = getSqlWhereItems(query, entityClassInfo, null, true, false, false);
        ...
        for (String fieldItem : entityClassInfo.getFieldMappingColumnMap().keySet()) {
            ...
            sqlValue.append(entitySimpleName).append(".").append(entityClassInfo.getFieldMappingColumnMap().get(fieldItem)).append("=").append(getInsertValueString(valueInfo)).append(",");
        }
        sqlValue.deleteCharAt(sqlValue.length() - 1);
    ...
    public <M extends WTModel> String getSqlOfDelete(WTQuery query, Class<M> modelClass) throws NoSuchFieldException, ClassNotFoundException, WTException {
        ...
        return "delete " + entityClassInfo.getEntityClass().getSimpleName() + " " + getSqlOfFromWhereGroupOrder(query, modelClass);
    }
    ...
    private <M extends WTModel> String getSqlOfFromWhereGroupOrder(WTQuery query, Class<M> modelClass) throws NoSuchFieldException, ClassNotFoundException, WTException {
        ...
        StringBuilder sqlValue = new StringBuilder(" from ").append(entityClassInfo.getTableName()).append(" ").append(entitySimpleName);
     ...
     * @return
     */
    private <E extends WTModel> TwoTuple<String, String> getSqlWhereItems(WTQuery query, WTEntityClassInfo entityClassInfo, StringBuilder sqlOfLeftJoinItems, Boolean generateLeftJoinSql, Boolean checkGroupBy, Boolean checkOrderBy) throws NoSuchFieldException, ClassNotFoundException, WTException {
        if (query == null) {
            return Tuple.tuple("", "");
        }
        ...
        for (Field fieldOfQuery : ReflectUtils.singleton.getFieldsWithInherit(query.getClass())) {
        ...
        if (checkOrderBy && query.getOrderList() != null && !query.getOrderList().isEmpty()) {
            ...
            for (WTOrder item : query.getOrderList()) {
                sqlOfOrderByItems.append(",")
                        ...
                        .append(".")
                        .append(entityClassInfo.getFieldMappingColumnMap().get(item.getFieldName()))
                        .append(" ")
    ...
    public <M extends WTModel> String getIncrementOrDecrementSql(String fieldName, WTQuery query, Class<M> modelClass, Boolean isIncrement) throws NoSuchFieldException, ClassNotFoundException, WTException {
        ...
        // 将操作的更新时间设置为当前时间
        updateSqlValue.append(", update_date = '").append(DateUtils.singleton.getDateTimeString(new Date())).append("'");
