...
import com.automonia.core.annotation.entity.WTField;
import com.automonia.core.annotation.query.WTSearch;
import com.automonia.core.annotation.query.WTSearchType;
...
import com.automonia.core.base.query.WTOrder;
import com.automonia.core.base.query.WTQuery;
import com.automonia.core.base.tuple.Tuple;
import com.automonia.core.base.tuple.TwoTuple;
import com.automonia.core.utils.DateUtils;
...
public class DataBaseFactory {
    ...
    @Autowired
    private EntityClassInfoFactory entityClassInfoFactory;
     ...
     */
    public <M extends WTModel> String getSqlOfSelectList(List<String> fieldNames, WTQuery query, Class<M> modelClass) throws WithoutEntityAnnotationException, BusinessException, NotSupportedTypeException, NoSuchFieldException, ClassNotFoundException, EntityWithoutTableAnnotationException, ParameterIsNullException {
        if (modelClass == null) {
            throw new BusinessException("未指定数据模型类");
        }
        ...
        if (modelClass != entityClass) {
            for (Field fieldOfModelClass : modelClass.getDeclaredFields()) {
                ...
                if (selectItemsIsEmpty || fieldNames.contains(fieldOfModelClass.getName())) {
                    ...
                    String fieldEntityName = StringUtils.isEmpty(wtFieldAnnotation.entityName()) ? entitySimpleName : wtFieldAnnotation.entityName();
                    String fieldName = StringUtils.isEmpty(wtFieldAnnotation.fieldName()) ? fieldOfModelClass.getName() : wtFieldAnnotation.fieldName();
                    // 如果使用的三级表关联，那么第三级表的别名由entityName + midJoinOn决定，如果使用的二级表关联，那么第二级表的别名由entityName + fromOn决定
                    String entityNameFronOn = StringUtils.isEmpty(wtFieldAnnotation.midEntityName()) ? wtFieldAnnotation.fromOn() : wtFieldAnnotation.midJoinOn();
                    ...
                    // 记录使用到的第三方关联表
                    sqlOfLeftJoinItems.append(getLeftJoinItemSql(wtFieldAnnotation, entitySimpleName, sqlOfLeftJoinItems));
                }
            }
        }
         ...
         */
        sqlValue.append(" from ").append(getEntityClassInfoFactory().getTableName(entitySimpleName)).append(" ").append(entitySimpleName).append(sqlOfLeftJoinItems);
        ...
        second: 关联语句
         */
        TwoTuple whereAndLeftJoinItemsSql = getSqlWhereItems(query, entityClass, true, true, true);

        return sqlValue.append(whereAndLeftJoinItemsSql.second).append(whereAndLeftJoinItemsSql.first).toString();
    }

    public <M extends WTModel> String getSqlOfSelectList(WTQuery query, Class<M> modelClass) throws WithoutEntityAnnotationException, BusinessException, NotSupportedTypeException, NoSuchFieldException, ClassNotFoundException, EntityWithoutTableAnnotationException, ParameterIsNullException {
        return getSqlOfSelectList(null, query, modelClass);
    }


    public <M extends WTModel> String getSqlOfSelectPage(WTQuery query, Class<M> modelClass) throws WithoutEntityAnnotationException, BusinessException, NotSupportedTypeException, NoSuchFieldException, ClassNotFoundException, EntityWithoutTableAnnotationException, ParameterIsNullException {
        return getSqlOfSelectPage(null, query, modelClass);
    }

    public <M extends WTModel> String getSqlOfSelectPage(List<String> fieldNames, WTQuery query, Class<M> modelClass) throws WithoutEntityAnnotationException, BusinessException, NotSupportedTypeException, NoSuchFieldException, ClassNotFoundException, EntityWithoutTableAnnotationException, ParameterIsNullException {
        return getSqlOfSelectList(fieldNames, query, modelClass) + getLimitSql(query);
    }

    public <M extends WTModel> String getSqlOfSelectOne(WTQuery query, Class<M> modelClass) throws EntityWithoutTableAnnotationException, WithoutEntityAnnotationException, ClassNotFoundException, BusinessException, NotSupportedTypeException, NoSuchFieldException, ParameterIsNullException {
        return getSqlOfSelectOne(null, query, modelClass);
    }

    public <M extends WTModel> String getSqlOfSelectOne(List<String> fieldNames, WTQuery query, Class<M> modelClass) throws EntityWithoutTableAnnotationException, WithoutEntityAnnotationException, ClassNotFoundException, BusinessException, NotSupportedTypeException, NoSuchFieldException, ParameterIsNullException {
        return getSqlOfSelectList(fieldNames, query, modelClass) + " limit 1";
    }

    public <M extends WTModel> String getSqlOfSelectCount(WTQuery query, Class<M> modelClass) throws EntityWithoutTableAnnotationException, WithoutEntityAnnotationException, ClassNotFoundException, BusinessException, NotSupportedTypeException, NoSuchFieldException, ParameterIsNullException {
        return "select count(*)" + getSqlOfFromWhereGroupOrder(query, modelClass);
    }

    public String getSqlOfInsert(WTModel model) throws WithoutEntityAnnotationException, NoSuchFieldException, NotSupportedTypeException, EntityWithoutTableAnnotationException, BusinessException, ParameterIsNullException, ClassNotFoundException {
        if (model == null) {
            throw new BusinessException("数据模型类是null");
        }
    ...
    public String getSqlOfUpdate(WTQuery query, WTModel model) throws BusinessException, WithoutEntityAnnotationException, EntityWithoutTableAnnotationException, NotSupportedTypeException, NoSuchFieldException, ClassNotFoundException, ParameterIsNullException {
        if (model == null) {
            throw new BusinessException("数据模型类是null");
        }
        ...
        TwoTuple whereAndLeftJoinItemsSql = getSqlWhereItems(query, entityClass, true, false, false);
    ...
    public <M extends WTModel> String getSqlOfDelete(WTQuery query, Class<M> modelClass) throws BusinessException, WithoutEntityAnnotationException, EntityWithoutTableAnnotationException, NotSupportedTypeException, NoSuchFieldException, ClassNotFoundException, ParameterIsNullException {
        return "delete" + getSqlOfFromWhereGroupOrder(query, modelClass);
    }
     ...
     * @return
     */
    private String getLeftJoinItemSql(WTSearch searchAnnotation, String entityName, StringBuilder leftJoinSqlValue) throws ClassNotFoundException, EntityWithoutTableAnnotationException, ParameterIsNullException, BusinessException {
        if (searchAnnotation == null || StringUtils.isEmpty(entityName)) {
            throw new ParameterIsNullException("@WTSearch注解对象为空, 实体类名为空");
        }
        return getLeftJoinItemSql(leftJoinSqlValue, entityName, searchAnnotation.entityName(), searchAnnotation.fromOn(), searchAnnotation.joinOn(), searchAnnotation.midEntityName(), searchAnnotation.midFromOn(), searchAnnotation.midJoinOn());
    }

    private String getLeftJoinItemSql(WTField fieldAnnotation, String entityName, StringBuilder leftJoinSqlValue) throws ParameterIsNullException, BusinessException, EntityWithoutTableAnnotationException, ClassNotFoundException {
        if (fieldAnnotation == null || StringUtils.isEmpty(entityName)) {
            throw new ParameterIsNullException("@WTField注解对象为空, 实体类名为空");
        }
        return getLeftJoinItemSql(leftJoinSqlValue, entityName, fieldAnnotation.entityName(), fieldAnnotation.fromOn(), fieldAnnotation.joinOn(), fieldAnnotation.midEntityName(), fieldAnnotation.midFromOn(), fieldAnnotation.midJoinOn());
    }
     ...
     * @return
     */
    private String getLeftJoinItemSql(StringBuilder leftJoinSqlValue, String entitySimpleName, String entityName, String fromOn, String joinOn, String midEntityName, String midFromOn, String midJoinOn) throws ParameterIsNullException, BusinessException, EntityWithoutTableAnnotationException, ClassNotFoundException {
        ...
        // 三级关联
        if (!StringUtils.isEmpty(midEntityName)) {
            String midEnittyNameAslias = midEntityName + fromOn;
            if (leftJoinSqlValue.indexOf(midEntityName + fromOn) < 0) {
                sqlValue.append(" left join ").append(getEntityClassInfoFactory().getTableName(midEntityName))
                        .append(" ")
        ...
        if (leftJoinSqlValue.indexOf(entitySimpleName + fromOn) < 0) {
            // 都需要处理的二级查询
            sqlValue.append(" left join ")
     ...
     * @throws ClassNotFoundException
     */
    private <M extends WTModel> String getSqlOfFromWhereGroupOrder(WTQuery query, Class<M> modelClass) throws BusinessException, WithoutEntityAnnotationException, EntityWithoutTableAnnotationException, NotSupportedTypeException, NoSuchFieldException, ClassNotFoundException, ParameterIsNullException {
        if (modelClass == null) {
     ...
     * @return
     */
    private <E extends WTModel> TwoTuple<String, String> getSqlWhereItems(WTQuery query, Class<E> entityClass, Boolean generateLeftJoinSql, Boolean checkGroupBy, Boolean checkOrderBy) throws NotSupportedTypeException, NoSuchFieldException, ClassNotFoundException, EntityWithoutTableAnnotationException, BusinessException, ParameterIsNullException {
        if (query == null) {
            return Tuple.tuple("", "");
        }
        ...
        String entitySimpleName = entityClass.getSimpleName();
        StringBuilder leftJoinSqlValue = new StringBuilder();
        ...
        for (Field fieldOfQuery : queryClass.getDeclaredFields()) {
            ...
            if (fieldValueInfo == null || (StringUtils.isEmpty(StringUtils.getString(fieldValueInfo.first)) && searchAnnotation.type() != WTSearchType.nullable)) {
                continue;
            }
            String fieldEntityName = StringUtils.isEmpty(searchAnnotation.entityName()) ? entitySimpleName : searchAnnotation.entityName();
            String fieldName = StringUtils.isEmpty(searchAnnotation.fieldName()) ? fieldOfQuery.getName() : searchAnnotation.fieldName();

            if (!searchSqlMap.containsKey(searchAnnotation.group())) {
                searchSqlMap.put(searchAnnotation.group(), new StringBuilder());
            }
            searchSqlMap.get(searchAnnotation.group()).append(StringUtils.isEmpty(searchAnnotation.group()) ? "and(" : "or(");
            searchSqlMap.get(searchAnnotation.group()).append(fieldEntityName).append(searchAnnotation.fromOn()).append(".").append(getEntityClassInfoFactory().getColumnName(fieldEntityName, fieldName)).append(searchAnnotation.type().getSearchValue(getDataBaseValueString(fieldValueInfo))).append(")");

            /*
            如果设置了generateLeftJoinSql，并且也设置了关联属性，那么生成其left join语句，并追加到leftJoinSqlValue
             */
            if (generateLeftJoinSql) {
                leftJoinSqlValue.append(getLeftJoinItemSql(searchAnnotation, entitySimpleName, leftJoinSqlValue));
            }
        }
        for (String searchSqlItemKey : searchSqlMap.keySet()) {
            StringBuilder searchSqlItem = searchSqlMap.get(searchSqlItemKey);
            sqlValue.append("and(").append(searchSqlItem.substring(StringUtils.isEmpty(searchSqlItemKey) ? 3 : 2)).append(")");
        }
        ...
        return Tuple.tuple(sqlValue.toString(), leftJoinSqlValue.toString());
    }


    public <M extends WTModel> String getIncrementOrDecrementSql(String fieldName, WTQuery query, Class<M> modelClass, Boolean isIncrement) throws WithoutEntityAnnotationException, EntityWithoutTableAnnotationException, NoSuchFieldException, NotSupportedTypeException, ClassNotFoundException, BusinessException, ParameterIsNullException {
        if (modelClass == null || StringUtils.isEmptyOr(fieldName)) {
            return null;
        }
    ...
    private <M> TwoTuple<Object, Class> getFieldValue(M model, String fieldName) {
        if (model == null || StringUtils.isEmpty(fieldName)) {
            return null;
        }
        try {
            Method getFieldMethod = model.getClass().getMethod(StringUtils.getGetMethod(fieldName));
    ...
    private TwoTuple<String, Boolean> getValueString(Object value, Class valueTypeClass) {
         ...
         */
            ...
            return Tuple.tuple(((BaseEnum) value).getValue().toString(), false);
        }
